...
<div class="w-full min-h-screen p-4 sm:p-6 lg:p-8 bg-slate-50">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-2 bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm">
            <a href="{{ route('admin.cocina.index', ['area' => 'cocina']) }}"
               class="px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider transition-all {{ $areaSeleccionada === 'Cocina' ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20' : 'text-slate-400 hover:text-slate-800' }}">
                <i class="fas fa-fire mr-2"></i>Cocina
            </a>
            <a href="{{ route('admin.cocina.index', ['area' => 'barra']) }}"
               class="px-5 py-2.5 rounded-xl font-black text-xs uppercase tracking-wider transition-all {{ $areaSeleccionada === 'Barra' ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20' : 'text-slate-400 hover:text-slate-800' }}">
        ...
        <div class="flex items-center gap-2 text-xs font-black text-emerald-600 uppercase tracking-wider">
            ...
            <a href="{{ route('admin.cocina.historial', ['area' => $areaSeleccionada]) }}"
               class="ml-3 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 bg-white text-slate-500 hover:border-blue-200 hover:text-blue-600 hover:bg-blue-50 transition-all font-black text-[10px] uppercase tracking-wider shadow-sm"
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mb-6">
        ...
        <div class="relative p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            <div class="absolute inset-x-0 top-0 h-1 bg-blue-500"></div>
            <span class="text-[10px] font-black uppercase text-slate-400 tracking-wider">Órdenes Activas</span>
            <h4 id="stat-ordenes-activas" class="text-2xl sm:text-3xl font-black text-slate-800 mt-1">{{ $ordenesActivasEnArea }}</h4>
        </div>

        {{-- En Proceso - Púrpura --}}
        <div class="relative p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            <div class="absolute inset-x-0 top-0 h-1 bg-violet-500"></div>
            <span class="text-[10px] font-black uppercase text-slate-400 tracking-wider">En Proceso</span>
            <h4 id="stat-enproceso" class="text-2xl sm:text-3xl font-black text-slate-800 mt-1">{{ $enProceso }}</h4>
        </div>

        {{-- Listas (Turno) - Esmeralda --}}
        <div class="relative p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            <div class="absolute inset-x-0 top-0 h-1 bg-emerald-500"></div>
            <span class="text-[10px] font-black uppercase text-slate-400 tracking-wider">Listas (Turno)</span>
            <h4 id="stat-servidas" class="text-2xl sm:text-3xl font-black text-slate-800 mt-1">{{ $servidas }}</h4>
        </div>
...
<script>
    ...
    async function cargarKdsMesas() {
        try {
            ...
            if (mesas.length === 0) {
                list.innerHTML = `
                    <div class="text-[11px] text-slate-400 font-medium p-4 text-center bg-slate-50 rounded-xl border border-slate-200">
            ...
            mesas.forEach(m => {
                ...
                a.className = 'block p-4 rounded-xl border border-slate-200 bg-white hover:border-blue-200 hover:bg-blue-50/50 transition-all flex items-center justify-between group cursor-pointer shadow-sm';
                
                a.innerHTML = `
                    <div class="flex-1 w-full min-w-0 pr-2">
                        <div class="flex items-center gap-2 mb-1">
                            <h5 class="text-sm font-black text-slate-800 truncate group-hover:text-blue-600 transition-colors">
                                Mesa ${m.numero}
                            </h5>
                            <span class="inline-flex items-center px-1.5 py-0.5 rounded border border-emerald-100 bg-emerald-50 text-emerald-600 text-[8px] font-black uppercase tracking-wider">
                                ACTIVA
                            </span>
                        </div>
                        <div class="flex flex-wrap items-center gap-3 text-[10px] text-slate-400 font-bold">
                            <span class="flex items-center gap-1.5"><i class="fas fa-users text-blue-500"></i> ${m.capacidad ?? '0'} pax</span>
                            <span class="text-emerald-600 tracking-wide">$ ${Number(m.total_consumo || 0).toFixed(2)}</span>
                        </div>
                    </div>
                    <div class="pl-2 flex items-center shrink-0">
                        <div class="w-8 h-8 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center group-hover:bg-blue-600 group-hover:border-blue-600 transition-colors">
                            <i class="fas fa-chevron-right text-[9px] text-slate-400 group-hover:text-white transition-colors"></i>
                        </div>
                    </div>
                `;
                list.appendChild(a);
    ...
    // Manejo de colores de alerta según tiempo transcurrido
    ...
    document.getElementById('comandas-container').addEventListener('click', async (e) => {
        ...
        try {
            ...
            if (res.ok && data.success) {
                ...
                if (listo) {
                    nombre?.classList.add('line-through', 'opacity-40', 'text-slate-400');
                    nombre?.classList.remove('text-slate-800');
                    btn.classList.add('bg-emerald-600', 'border-emerald-600', 'text-white', 'scale-95');
                    btn.classList.remove('border-slate-300', 'text-slate-400');
                } else {
                    nombre?.classList.remove('line-through', 'opacity-40', 'text-slate-400');
                    nombre?.classList.add('text-slate-800');
                    btn.classList.remove('bg-emerald-600', 'border-emerald-600', 'text-white', 'scale-95');
