...
@section('title', 'Corte Semanal | Finanzas | Ollintem Pro')
@section('header-title', 'Corte Semanal')
@section('header-subtitle', 'Análisis detallado por rango de fechas')
...
<div class="p-3 sm:p-6 lg:p-8 xl:p-12 max-w-[1800px] mx-auto w-full space-y-5 sm:space-y-8 flex-1 flex flex-col transition-all duration-300 relative z-10">
    ...
    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-3 sm:gap-6 mb-2 animate-fade-in-up" style="animation-delay: 0ms;">
        <div class="space-y-1 w-full sm:w-auto">
            <h1 class="text-xl sm:text-3xl md:text-4xl font-black text-slate-900 tracking-tight">Corte Semanal</h1>
            <p class="text-xs sm:text-sm font-medium text-slate-500">
                Desglose diario de ingresos y egresos |
                <span class="text-slate-700 font-bold">
                    Del {{ \Carbon\Carbon::parse($fechaInicio)->translatedFormat('d M') }} al {{ \Carbon\Carbon::parse($fechaFin)->translatedFormat('d M, Y') }}
                </span>
            </p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-center gap-2 sm:gap-3 w-full xl:w-auto">
            ...
            <a href="{{ route('admin.finanzas.corte.semanal.exportar', ['fecha_inicio' => $fechaInicio, 'fecha_fin' => $fechaFin]) }}"
            ...
            <a href="{{ route('admin.finanzas.corte.semanal.pdf', ['fecha_inicio' => $fechaInicio, 'fecha_fin' => $fechaFin]) }}"
    ...
    {{-- FILTRO POR RANGO DE FECHAS (Semana) --}}
    <form method="GET" action="{{ route('admin.finanzas.corte.semanal') }}"
        ...
        <div class="flex-1 sm:max-w-[220px]">
            <label class="block text-xs font-bold text-slate-500 uppercase mb-1.5 ml-1">Fecha Inicio</label>
            <input type="date" name="fecha_inicio" value="{{ $fechaInicio }}" 
                class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-400 transition-all cursor-pointer">
        </div>

        <div class="flex-1 sm:max-w-[220px]">
            <label class="block text-xs font-bold text-slate-500 uppercase mb-1.5 ml-1">Fecha Fin</label>
            <input type="date" name="fecha_fin" value="{{ $fechaFin }}" 
                class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 focus:bg-white focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-400 transition-all cursor-pointer">
        </div>

        <button type="submit"
            class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-bold transition-all shadow-md shadow-indigo-500/20 flex items-center justify-center gap-2 hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98]">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-5">
        ...
        <div class="col-span-2 lg:col-span-1 bg-white border border-slate-100 rounded-2xl sm:rounded-[1.5rem] p-5 shadow-sm hover:shadow-lg hover:shadow-blue-900/5 transition-all duration-300 hover:-translate-y-1 relative overflow-hidden group animate-fade-in-up" style="animation-delay: 400ms;">
            ...
            <p class="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mt-1 group-hover:text-slate-600 transition-colors">Balance</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="space-y-1.5">
                ...
                @empty
                    <div class="py-4 text-center">
                        <p class="text-sm text-slate-400 font-medium">Sin ingresos en este rango.</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-100 rounded-2xl sm:rounded-[1.5rem] p-5 sm:p-6 shadow-sm hover:shadow-md transition-shadow animate-fade-in-up" style="animation-delay: 600ms;">
            ...
            <div class="space-y-1.5">
                ...
                @empty
                    <div class="py-4 text-center">
                        <p class="text-sm text-slate-400 font-medium">Sin egresos en este rango.</p>
                    </div>
                @endforelse
            </div>
        </div>
