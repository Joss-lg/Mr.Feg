{{-- resources/views/admin/fidelidad/modal-eliminar.blade.php --}}
<div id="modalEliminarNivel" class="hidden fixed inset-0 z-[9999] items-center justify-center bg-slate-900/40 backdrop-blur-sm transition-all duration-300 p-4">
    <div class="fixed inset-0 bg-transparent" onclick="window.closeDeleteNivelModal()"></div>

    <div id="deleteNivelContainer" class="relative bg-white border border-slate-200 rounded-[2rem] w-full max-w-md shadow-2xl scale-95 opacity-0 transition-all duration-300 overflow-hidden flex flex-col">
        ...
        <div class="p-6 sm:p-8 text-center space-y-4">
            ...
            <div class="space-y-2">
                <h2 class="text-xl sm:text-2xl font-black text-slate-800 tracking-tight">¿Eliminar nivel?</h2>
                <p class="text-xs sm:text-sm font-medium text-slate-500 leading-relaxed px-2 break-words">
                    Vas a eliminar permanentemente el nivel <span id="delete_nivel_nombre_display" class="text-slate-900 font-black underline decoration-rose-500/40 decoration-2"></span> del sistema. Esta acción no se puede revertir.
                </p>
            </div>
        </div>

        {{-- Formulario de Acción (submit normal, no AJAX) --}}
        <form id="formEliminarNivel" method="POST" action="" class="flex flex-col flex-1 min-h-0 bg-slate-50">
            @csrf
            @method('DELETE')

            <div class="flex flex-col-reverse sm:flex-row items-center gap-3 px-6 sm:px-8 pb-6 sm:pb-8 pt-6 border-t border-slate-100">
                <button type="button" onclick="window.closeDeleteNivelModal()"
